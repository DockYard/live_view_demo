  ...
  defp paths(%{"paths" => paths, "text_paths" => text_paths}) do
    ...
    |> Enum.map(fn {path, index} ->
      %TypoKart.Path{
        ...
        chars:
          Enum.at(text_paths, index) |> Map.get("chars") |> String.trim() |> String.to_charlist(),
    ...
    |> Enum.map(
      &{
        struct(PathCharIndex,
          path_index: Enum.at(&1, 0) |> Map.get("path_index"),
          char_index: Enum.at(&1, 0) |> Map.get("char_index")
        ),
        struct(PathCharIndex,
          path_index: Enum.at(&1, 1) |> Map.get("path_index"),
          char_index: Enum.at(&1, 1) |> Map.get("char_index")
        )
      }
    )
  end
